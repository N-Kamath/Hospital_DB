...
public class DoctorDao {
	...
	public boolean registerDoctor(Doctor d) {
		...
		try {
			String sql = "insert into doctorview1(DOCTORID,DDOB,DOCLICENSEID,SHIFTID,DEMAIL,DNAME,QUALIFICATION,SPECIALIZATION,DPASS,DPHONE) values(?,?,?,?,?,?,?,?,?,?)";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, 51);
			ps.setString(2, d.getDob());
			ps.setString(3, d.getLicenseID());
			ps.setInt(4, Integer.parseInt(d.getShiftID()));
			ps.setString(5, d.getEmail());
			ps.setString(6, d.getFullName());
			ps.setString(7, d.getQualification());
			ps.setString(8, d.getSpecialist());
			ps.setString(9, d.getPassword());
			ps.setInt(10, Integer.parseInt(d.getMobNo()));
	...
	public Doctor login(String email, String psw) {
		...
		try {
			...
			String sql = "select * from doctor where DEMAIL=? and DPASS=?";
			PreparedStatement ps = conn.prepareStatement(sql);
